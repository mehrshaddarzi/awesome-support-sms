...
class Utility {
	...
	public static function send_mail( $to, $subject, $content ) {
		...
		//Email Template
		$email_template = wp_normalize_path( \AW_SMS::$plugin_path . '/templates/email.php' );
		if ( trim( \AW_SMS::$Template_Engine ) != "" ) {
			$template = wp_normalize_path( path_join( get_template_directory(), '/aw-sms/email.php' ) );
		...
		$opt = get_option( 'aw_sms_email_opt' );
